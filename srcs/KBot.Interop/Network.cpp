...
Network::Network()
{
	recv = Module::GetInstance()->FindPattern<DWORD>("\x55\x8B\xEC\x83\xC4\xF4\x53\x56\x57\x33\xC9\x89\x4D\xF4\x89\x55\xFC\x8B\xD8\x8B\x45\xFC\xE8\x00\x00\x00\x00\x33\xC0", "xxxxxxxxxxxxxxxxxxxxxxx????xx", 0);
	send = Module::GetInstance()->FindPattern<DWORD>("\x53\x56\x8B\xF2\x8B\xD8\xEB\x04", "xxxxxxxx", 0);
	caller = *Module::GetInstance()->FindPattern<DWORD*>("\xA1\x00\x00\x00\x00\x8B\x00\xBA\x00\x00\x00\x00\xE8\x00\x00\x00\x00\xE9\x00\x00\x00\x00\xA1\x00\x00\x00\x00\x8B\x00\x8B\x40\x40", "x????xxx????x????x????x????xxxxx", 1);
}
